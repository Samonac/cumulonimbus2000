...
lightDebug = False
sleepOffset = 0.1
lightDict = { # TODO : Make all these settings somewhat automatic
    "Chambre - Pyramide": 0,
...
lightsToSwitchPerRoomDict = {
    ...
    'ZONE Salle à manger': ["Hue white spot 1", "Hue white spot 2", "Hue white spot 3", "Hue white spot 4", "Salon - Nautilus"],
...
filename = 'data/phue_config/phue_key.json'
with open(filename, "r") as jsonFile:
    jsonFileStr = jsonFile.read()
    store = json.loads(jsonFileStr)
bridge_ip = store['bridge_ip']
...
def saveJson(jsonOutput, fileName):
    ...
    jsonArray = glob.glob('data/phue_config/lightData/json/*.json')
    ...
    if len(jsonArray) > 0:
        ...
        if not jsonEquals(dataPast, jsonOutput):
            ...
            with open('data/phue_config/lightData/{}.json'.format(fileName), "w") as outfile:
    ...
    with open('data/phue_config/lightData/json/{}.json'.format(fileName), "w") as outfile:
        outfile.write(json_object)
    return True
...
def main_calcifer(taskArray):
    nextSpeedModeTemp = { taskArray[0] }
    print('taskArray[0] : ', taskArray[0])
    print('of type : ', type(taskArray[0]))
    if taskArray[0].nextSpeedMode:
        nextSpeedMode = taskArray[0]['nextSpeedMode']
        if lightDebug or specialDebug: setSpecificLightBrightness("Chevet Antoine", 1)
        # saveLightContext
        ...
        if actions != {}:
            ...
            if lightDebug or specialDebug: setSpecificLightBrightness("Chevet Antoine", 70)
            nextSpeedMode = True
            doActions(actions)
        else:
            print('No actions : sleeping for ', sleepOffset)

            if lightDebug or specialDebug: setSpecificLightBrightness("Chevet Antoine", 33)
            nextSpeedMode = True
        ...
        # fluctuateLights()
        # time.sleep(2)
    elif taskArray[0]['Chevet Antoine']:
        setSpecificLightBrightness("Chevet Antoine", taskArray[0]['Chevet Antoine']['brightness'])
    else:
        print('nothing to do ?')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        main_calcifer(sys.argv[1])
    else:
        main_calcifer(taskArray=[{'Chevet Antoine': {'brightness': 50, 'color': [255, 255, 255]}}])
        print("Usage: python calcifer.py <taskArray=[{'Chevet Antoine': {'brightness': 50, 'color': [255, 255, 255]}}]>")
